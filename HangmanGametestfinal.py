...
class PlayPage(tk.Frame):
    ...
    def drawGuessWord(self):
        ...
        xcenter = int((int(self.winfo_screenwidth()/2 - self.winfo_reqwidth()/2)- (len(self.words)* 58))/2) + 254/2
        for col in range(len(self.words)):
            ...
            self.btn_guess[col].place(x= xcenter + (58 * col), y=90)
    ...
    def PlayAgain(self):
        ...
        self.lbLife.configure(text = txt)
        self.clearGuessWord()
        self.words = self.getWord()
        self.drawGuessWord()
        for col in range(len(self.words)):
    ...
    def result(self, gameOver):
        if gameOver == True:
            ...
            #print("Win")
            self.lbResult.configure(text ="EXCELLENT!!!")
...
class RecordPage(tk.Frame):
    ...
    def searchRefresh(self):
        ...
        self.lbResult.configure(text=" ")
        self.clearTreeview()
        list_sort = sorted(self.lst_record, key=lambda k: k['name'])
        for x in list_sort:
            self.tree.insert("",'end',text="L1",values=(x["name"], x["score"]))
app = Hangman()
app.mainloop()
